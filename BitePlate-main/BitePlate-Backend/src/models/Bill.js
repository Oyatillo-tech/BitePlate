...
class BillModel {
    static async create(orderId, pricingData) {
        try {
            const result = await pool.query(
                `INSERT INTO bills 
                (order_id, subtotal, tax, discount, total, final_total, status)
    ...
    static async getSplits(billId) {
        try {
            const result = await pool.query(
                'SELECT * FROM bill_splits WHERE bill_id = $1 ORDER BY id',
                [billId]
            );
            return result.rows;
        } catch (err) {
    ...
    static async splitBill(billId, splits) {
        ...
        try {
            ...
            if (bill.status !== 'PENDING') {
                throw new Error('Only pending bills can be split');
            }

            const billTotal = parseFloat(bill.final_total || bill.total);
            const splitTotal = splits.reduce((sum, s) => sum + parseFloat(s.amount), 0);

            if (Math.abs(splitTotal - billTotal) > 0.01) {
                throw new Error(`Split amounts must equal bill total (£${billTotal.toFixed(2)})`);
            }
            ...
            for (const split of splits) {
                const result = await client.query(
                    `INSERT INTO bill_splits (bill_id, label, amount, payment_method, status)
                     VALUES ($1, $2, $3, $4, 'PENDING')
                     RETURNING *`,
                    [billId, split.label || 'Guest', split.amount, split.paymentMethod || null]
                );
                createdSplits.push(result.rows[0]);
            }

            await client.query(
                `UPDATE bills SET status = 'SPLIT' WHERE id = $1`,
                [billId]
            );
    ...
    static async paySplit(splitId, paymentMethod) {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');

            const splitResult = await client.query(
                'SELECT * FROM bill_splits WHERE id = $1',
                [splitId]
            );
            ...
            await client.query(
                `UPDATE bill_splits SET status = 'PAID', payment_method = $1 WHERE id = $2`,
                [paymentMethod, splitId]
            );
            ...
            if (parseInt(pendingResult.rows[0].count) === 0) {
                ...
                await client.query(
                    `UPDATE bills SET status = 'PAID', payment_method = 'SPLIT' WHERE id = $1`,
                    [split.bill_id]
                );
