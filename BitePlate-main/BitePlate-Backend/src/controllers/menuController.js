...
import ComboMealModel from '../models/ComboMeal.js';
...
export const getMenuByType = async (req, res) => {
    try {
        ...
        const validTypes = ['STARTER', 'MAIN', 'DESSERT', 'BEVERAGE', 'COMBO'];
...
export const getComboMeals = async (req, res) => {
    try {
        const combos = await ComboMealModel.getAll();
        res.json({
            success: true,
            data: combos
        });
...
export const getComboMeal = async (req, res) => {
    try {
        const { id } = req.params;
        const combo = await ComboMealModel.getById(id);
...
export const createComboMeal = async (req, res) => {
    try {
        const { name, description, price, items } = req.body;

        if (!name || price === undefined || !items || items.length === 0) {
            return res.status(400).json({
                success: false,
                error: 'Name, price, and items required'
            });
        }

        const combo = await ComboMealModel.create({ name, description, price, items });
